...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot, num_intent):
        ...
        if args.use_pretrained:
            ...
            self.slot_decoder = LSTMDecoder(
                self.args.encoder_hidden_dim + 128,
        ...
        self.intent2slot_att = QKVAttention(
            query_dim=self.args.encoder_hidden_dim,
            key_dim=self.num_intent,
            value_dim=self.num_intent,
            hidden_dim=128,
            output_dim=128,
    ...
    def forward(self, text, seq_lens, kg_var, kg_lens, kg_count, up_var, ca_var, n_predicts=None, forced_slot=None):
        ...
        # 👇 联合注意力（创新点）
        intent_context = self.intent2slot_att(hiddens, feed_intent, feed_intent)

        # 👇 拼接 context，形成 slot decoder 输入
        slot_input = torch.cat([hiddens, intent_context], dim=-1)
